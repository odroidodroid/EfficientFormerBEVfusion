...
from mmdet3d.models.utils import FFN, PositionEmbeddingLearned, TransformerDecoderLayer, PoolFormer, ConvFormer
...
class TransFusionHead(nn.Module):
    def __init__(
        ...
        self.class_encoding = nn.Conv1d(num_classes, hidden_channel, 1) ## num_class = 10
    ...
    def get_bboxes(self, preds_dicts, metas, img=None, rescale=False, for_roi=False):
        ...
        for layer_id, preds_dict in enumerate(preds_dicts):
            ...
            for i in range(batch_size):
                ...
                # iou_rectifier = scores.new_tensor([0.5])
...
@HEADS.register_module()
class PoolFormerHead(nn.Module):
    def __init__(
        self,
        num_proposals=128,
        auxiliary=True,
        ...
        super(PoolFormerHead, self).__init__()
    ...
    def __del__(self):
        # super(PoolFormerHead, self).__del__()
        global output
        import pandas as pd
        df = pd.DataFrame.from_dict(output)
        df.to_pickle("poolformer_out.pkl")
        print("poolformer_out.pkl saved")
    ...
    def forward_single(self, inputs, img_inputs, metas):
        ...
        # TMP Transformer output dist
        # printed at __del__
        global output
        detached_dict = {key: value.detach().cpu().numpy() for key, value in ret_dicts[-1].items()}\
        # too big to save
        del detached_dict['dense_heatmap']
        output.append(detached_dict)
...
@HEADS.register_module()
class ConvFormerHead(nn.Module):
    def __init__(
        self,
        num_proposals=128,
        auxiliary=True,
