...
class DynamicPillarVFE(VFETemplate):
    def __init__(self, 
        ...
        num_filters_list = [num_point_features] + list(self.num_filters)

        pfn_layers = []
        for i in range(len(num_filters_list) - 1):
            in_filters = num_filters_list[i]
            out_filters = num_filters_list[i + 1]
            pfn_layers.append(
                PFNLayerV2(in_filters, out_filters, self.use_norm, last_layer=(i >= len(num_filters_list) - 2))
            )
    ...
    def forward(self, points):
        ...
        for batch in range(batch_size):
            points_batch = torch.zeros((points[batch].shape[0],1))
            points_batch = points_batch.fill_(batch).to(device=points[batch].device)
            points_batch = torch.cat((points_batch, points[batch]), dim=1)
            points_.append(points_batch)
        points = torch.cat(points_, dim=0)
