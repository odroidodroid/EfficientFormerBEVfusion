...
from mmdet3d.models.backbones.dynamic_pillar_vfe import DynamicPillarVFE
...
class DSVTInputLayer(nn.Module):
    ...
    def forward(self, feats, coords):
        ...
        '''
        voxel_info = {}
        voxel_info['voxel_feats_stage0'] = feats.clone()
        voxel_info['voxel_coors_stage0'] = coords.clone()
...
class DSVT(nn.Module):
    ...
    def __init__(self,
                 ...
                 conv_out_channel,):
        super().__init__()
        self.vfe = DynamicPillarVFE(vfe["num_point_features"],
                                    vfe["voxel_size"],
                                    vfe["grid_size"],
                                    vfe["point_cloud_range"],
                                    vfe["use_norm"],
                                    vfe["with_distance"],
                                    vfe["use_absolute_xyz"],
                                    vfe["num_filters"])
        self.input_layer = DSVTInputLayer(input_layer["sparse_shape"],
                                          input_layer["window_shape"],
                                          input_layer["downsample_stride"],
                                          input_layer["d_model"],
                                          input_layer["set_info"],
                                          input_layer["hybrid_factor"],
                                          input_layer["shifts_list"],
                                          input_layer["normalize_pos"])
        ...
        for stage_id in range(stage_num):
            ...
            if stage_id < stage_num-1:
                downsample_window = self.input_layer.downsample_stride[stage_id]
    ...
    def forward(self, feats, batch_size=1):
        '''
        Args:
            bacth_dict (dict): 
                The dict contains the following keys
        ...
        Returns:
            bacth_dict (dict):
                ...
                - ...
        '''
        feats, coords = self.vfe(feats)
        voxel_info = self.input_layer(feats, coords)
        ...
        pillar_feats = feats = output
        voxel_coors = voxel_info[f'voxel_coors_stage{self.stage_num - 1}']
        return pillar_feats, voxel_coors
