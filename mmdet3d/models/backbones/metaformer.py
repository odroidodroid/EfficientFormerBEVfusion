...
class MetaFormer(BaseModule):
    ...
    def __init__(self, in_chans=3, num_classes=10, 
        ...
        self.num_features = [64, 128, 320, 512]
        self.height = [64, 32, 16, 8]
        self.width = [176, 88, 44, 22]
        ...
        super(MetaFormer, self).__init__(init_cfg=init_cfg)
        ...
        self.norm = [output_norm(i).to(device='cuda') for i in self.num_features]
    ...
    def init_weights(self):
        if self.init_cfg is not None :
            state_dict = torch.hub.load_state_dict_from_url(self.init_cfg.checkpoint,
...
class poolformerv2_s24(MetaFormer):
    def __init__(self, **kwargs) :
        super().__init__(depths=[4, 4, 12, 4],
                         dims=[64, 128, 320, 512],
                         token_mixers=Pooling,
                         norm_layers=partial(LayerNormGeneral, normalized_dim=(1, 2, 3), eps=1e-6, bias=False),
                         **kwargs)
...
class poolformerv2_s36(MetaFormer):
    def __init__(self, **kwargs) :
        super().__init__(depths=[6, 6, 18, 6],
                         dims=[64, 128, 320, 512],
                         token_mixers=Pooling,
                         norm_layers=partial(LayerNormGeneral, normalized_dim=(1, 2, 3), eps=1e-6, bias=False),
                         **kwargs)
...
class poolformerv2_m48(MetaFormer):
    def __init__(self, **kwargs) :
        super().__init__(depths=[8, 8, 24, 8],
                         dims=[96, 192, 384, 768],
                         token_mixers=Pooling,
                         norm_layers=partial(LayerNormGeneral, normalized_dim=(1, 2, 3), eps=1e-6, bias=False),
                         **kwargs)
...
class convformer_s36(MetaFormer):
    def __init__(self, **kwargs):
        super().__init__(depths=[3, 12, 18, 3],
                         dims=[64, 128, 320, 512],
                         token_mixers=SepConv,
                         head_fn=MlpHead,
                         **kwargs)
